...
import { findIdToResetPasswordApi, resetPasswordApi } from '../api';
...
export const ResetPasswordForm = () => {
  ...
  const { mutate: resetPasswordMutation } = useMutation({
    mutationFn: (dto: { username: string; password: string }) => resetPasswordApi(dto),
    onSuccess: () => {
      setIsLoading(false);
      setStep(3);
    },
