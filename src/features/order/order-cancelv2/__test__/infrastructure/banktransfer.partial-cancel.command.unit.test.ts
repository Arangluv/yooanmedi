...
import { BankTransferPartialCancelCommandDto } from '../../infrastructure';

describe('BankTransfer Partial Cancel Command', () => {
  ...
  describe('BankTransferPartialCancelCommandForImmediate', () => {
    let commandDto: BankTransferPartialCancelCommandDto;
    let command: BankTransferPartialCancelCommandForImmediate;

    beforeEach(() => {
      commandDto = { order: createOrderFixture(), orderProductId: 1 };
      command = new BankTransferPartialCancelCommandForImmediate(dependencies, commandDto);
    });

    it('무통장 입금 즉시 실행취소가 실행된다', async () => {
      // When
    ...
    it('포인트 사용 환불 내역이 생성된다', async () => {
      ...
      // Then
      const { repository } = dependencies;
      expect(repository.pointHistory.createRollbackHistory).toHaveBeenCalledTimes(1);
    });

    it('유저 포인트가 업데이트 된다', async () => {
      // Given
      const { repository } = dependencies;
    ...
    it('부분취소에 실패시 transaction이 rollback되고 BaseError를 throw한다', async () => {
      ...
      const command = new BankTransferPartialCancelCommandForImmediate(dependencies, commandDto);
  ...
  describe('BankTransferPartialCancelCommandForRequest', () => {
    let commandDto: BankTransferPartialCancelCommandDto;
    let command: BankTransferPartialCancelCommandForRequest;

    beforeEach(() => {
      commandDto = { order: createOrderFixture(), orderProductId: 1 };
      command = new BankTransferPartialCancelCommandForRequest(dependencies, commandDto);
    });

    it('무통장 입금 취소요청이 실행된다', async () => {
      // When
      const result = await command.execute();
    ...
    it('주문상품을 취소요청 상태로 업데이트 한다', async () => {
      // When
      await command.execute();
    ...
    it('주문상품이 모두 취소상태인 경우 취소요청상태, 전체결제취소 상태로 업데이트 된다', async () => {
      // Given
      const { repository } = dependencies;
      vi.mocked(repository.orderProduct.findMany).mockResolvedValue([
        createOrderProductFixture({ orderProductStatus: 'cancelled' }),
    ...
    it('주문상품이 모두 취소상태가 아닌 경우 취소요청상태, 부분결제취소 상태로 업데이트 된다', async () => {
      // Given
      const { repository } = dependencies;
      vi.mocked(repository.orderProduct.findMany).mockResolvedValue([
        createOrderProductFixture({ orderProductStatus: 'cancelled' }),
    ...
    it('부분취소요청에 성공하면 transaction이 commit된다', async () => {
      // When
      await command.execute();
    ...
    it('부분취소요청에 실패시 transaction이 rollback되고 BaseError를 throw한다', async () => {
      // Given
      const dependencies = MockCancelOrderServiceDependency.fail;
      const command = new BankTransferPartialCancelCommandForImmediate(dependencies, commandDto);
  ...
  describe('BankTransferPartialCancelCommandForPaied', () => {
    let commandDto: BankTransferPartialCancelCommandDto;
    let command: BankTransferPartialCancelCommandForPaied;
    ...
    beforeEach(() => {
      ...
      command = new BankTransferPartialCancelCommandForPaied(dependencies, commandDto);
    });

    it('무통장 입금 취소가 실행된다', async () => {
      // When
      const result = await command.execute();
    ...
    it('주문상품이 모두 취소상태인 경우 취소상태, 전체결제취소 상태로 업데이트 된다', async () => {
      // Given
      const { repository } = dependencies;
      vi.mocked(repository.orderProduct.findMany).mockResolvedValue([
        createOrderProductFixture({ orderProductStatus: 'cancelled' }),
    ...
    it('주문상품 중 취소 요청상태가 있는 경우 취소요청상태, 부분결제취소 상태로 업데이트 된다', async () => {
      // Given
      const { repository } = dependencies;
      vi.mocked(repository.orderProduct.findMany).mockResolvedValue([
    ...
    it('주문상품이 모두 취소상태가 아닌 경우 현재 주문의 상태, 부분결제취소 상태로 업데이트 된다', async () => {
      // Given
      const { repository } = dependencies;
      vi.mocked(repository.orderProduct.findMany).mockResolvedValue([
        createOrderProductFixture({ orderProductStatus: 'preparing' }),
