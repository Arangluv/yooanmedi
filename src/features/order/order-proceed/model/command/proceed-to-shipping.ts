...
export class ProceedToShippingCommand extends BaseProceedCommand {
  constructor() {
    super();
  }
  ...
  async runProceed(targetOrderId: number): Promise<CommandResult> {
    try {
      ...
      this.setOrderProductIds(
        await findOrderProductIdsByStatus(targetOrderId, ORDER_STATUS.PREPARING),
      );
      ...
      const errorMessage =
        error instanceof Error ? error.message : '배송중 상태로 변경하는데 실패했습니다';
      return {
        success: false,
