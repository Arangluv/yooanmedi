...
import {
  ...
  BankTransferTotalCancelCommandDto,
} from '../../infrastructure';
import { CancelOrderServiceDependencies } from '../../core';

describe('BankTransfer Total Cancel Command', () => {
  let dependencies: CancelOrderServiceDependencies;
  let commandDto: BankTransferTotalCancelCommandDto;
  let command: BankTransferTotalCancelCommand;
  ...
  beforeEach(() => {
    ...
    command = new BankTransferTotalCancelCommand(dependencies, commandDto);
  ...
  it('무통장 입금 전체 주문상품 결제 취소가 실행된다', async () => {
    // When
  ...
  it('부분취소에 실패시 transaction이 rollback되고 BaseError를 throw한다', async () => {
    ...
    const command = new BankTransferTotalCancelCommand(dependencies, commandDto);
