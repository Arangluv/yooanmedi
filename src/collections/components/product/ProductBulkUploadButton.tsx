...
export default function ProductBulkUploadButton() {
...
function ExcelUploadModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="flex min-h-[800px] w-2/3 flex-col gap-8 rounded-lg bg-white p-8"
...
function ProductList({ dtoData, onClose }: { dtoData: any[]; onClose: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="scrollbar-thin scrollbar-thumb-rounded-full scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-700 h-full max-h-[700px] w-full overflow-y-auto">
        <table className="w-full">
          ...
          <tbody>
            {dtoData.map((item, index) => (
              <tr className="text-[14px] text-black/70" key={index}>
                <td className="py-2 text-center">{index + 1}</td>
                <td className="text-center">{item.name}</td>
                <td className="text-center">{item.insurance_code}</td>
                <td className="max-w-[200px] overflow-hidden text-center text-ellipsis whitespace-nowrap">
                  {item.ingredient}
                </td>
