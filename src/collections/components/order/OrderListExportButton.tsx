...
export default function OrderListExportButton({ data }: { data: any[] }) {
  const handleExport = () => {
    ...
    // 가운데 정렬할 컬럼 인덱스 (병원명, 상품명, 규격, 결제방법, 주문상태, 요청사항)
    const centerAlignColumns = [2, 3, 6, 7, 9, 10] // 컬럼 인덱스: 병원명(2), 상품명(3), 규격(6), 결제방법(7), 주문상태(9), 요청사항(10)
  ...
  return (
    <div>
      <button
        className="px-3 py-1 rounded-md bg-[#257449] text-white cursor-pointer"
